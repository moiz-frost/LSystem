// An Generator has a starting sentence
// An a ruleset
...
class Generator {
  ...
  // Construct an Generator with a startin sentence and a ruleset
  Generator(String axiom, Rule[] r) {
    sentence = axiom;
    ruleset = r;
    generation = 0;
  }
  ...
  void generate() {
    ...
    for (int i = 0; i < sentence.length(); i++) {
      ...
      char curr = sentence.charAt(i);//A
      // We will replace it with itself unless it matches one of our rules
      String replace = "" + curr;//A
      // Check every rule
